    ...
    # classifier function
       ...
       # set labels to lowercase (lower) and stripping off whitespace(strip)
        # Get the classifier label
            predicted_label = classifier(images_dir + image_file, model).lower().strip()

            # Get the real label
            actual_label = results[image_file][0]

            # Compare labels and get prediction 
            ...
            results[image_file].append(predicted_label) 
            results[image_file].append(prediction)
